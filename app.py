...
# Constants
LEADERBOARD_FILE = get_resource_path("data/leaderboard.json")
GUEST_LEADERBOARD_FILE = get_resource_path("data/guest_leaderboard.json")
CHAPTERS_FILE = get_resource_path("data/chapters.json")

# Load game settings at startup
def load_initial_settings():
    """Load initial game settings or use defaults"""
    try:
        settings_file = get_resource_path('data/game_settings.json')
...
@app.route('/select_chapter_test')
def select_chapter_test():
    """Chapter selection for Test Yourself mode"""
    chapters_data = load_chapters()
    # Get unlocked chapters for test_yourself mode, sorted by order
    chapters = sorted(
        [ch for ch in chapters_data.get("chapters", []) 
         if not ch.get("locked_test_yourself", False) or not ch.get("locked_test_yourself")],
        key=lambda x: x.get("order", 0)
    )
    # Include locked chapters too for display
    all_chapters = sorted(chapters_data.get("chapters", []), key=lambda x: x.get("order", 0))
    return render_template('select_chapter_test.html', chapters=all_chapters)
...
try:
    with open(get_resource_path('data/enemies.json'), encoding='utf-8') as f:
        enemies = json.load(f)
...
def teacher_delete_chapter(chapter_id):
    try:
        ...
        original_count = len(data['chapters'])
        data['chapters'] = [chapter for chapter in data['chapters'] if chapter['id'] != chapter_id]
        
        if len(data['chapters']) == original_count:
            return jsonify({'success': False, 'error': f'Chapter {chapter_id} not found'})
        
        # Save updated chapters
        with open(get_resource_path('data/chapters.json'), 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        
        return jsonify({'success': True, 'message': f'Chapter {chapter_id} deleted successfully'})
    except Exception as e:
...
def teacher_delete_question(question_id):
    try:
        ...
        if len(all_questions) == original_count:
            return jsonify({'success': False, 'error': f'Question {question_id} not found'})
        
        # Save questions
        with open(get_resource_path('data/questions.json'), 'w', encoding='utf-8') as f:
            json.dump(all_questions, f, indent=2, ensure_ascii=False)
        
        return jsonify({'success': True, 'message': f'Question {question_id} deleted successfully'})
...
def teacher_add_question():
    try:
        ...
        question_type = request.form.get('question_type', 'short_answer').strip()
        
        # Get answer and additional data based on question type
        if question_type == 'true_false':
            answer = request.form.get('tf_answer', '').strip()
        elif question_type == 'multiple_choice':
            # Get the selected answer from radio button (A, B, C, or D)
            answer = request.form.get('mc_answer', '').strip()
        ...
        new_question = {
            ...
            "is_new": True,  # Tag to identify newly added questions
            "created_at": datetime.now().isoformat(),
            "updated_at": datetime.now().isoformat()
        }
        
        # Add multiple choice options if applicable
        if question_type == 'multiple_choice' and 'options' in locals():
    ...
    try:
        ...
        for question in questions:
            if question.get('id') == question_id:
                ...
                # Remove 'is_new' flag when editing (no longer new)
                question.pop('is_new', None)
                
                # Handle multiple choice options
                if question_type == 'multiple_choice' and 'options' in locals():
                    question['options'] = options
                elif question_type != 'multiple_choice':
                    question.pop('options', None)  # Remove options if not multiple choice
...
def load_levels_with_status():
    """Load levels data with status information"""
    try:
        levels_file = os.path.join(os.path.dirname(__file__), 'data', 'levels.json')
        with open(levels_file, 'r', encoding='utf-8') as f:
            levels_data = json.load(f)
            
        # Calculate level status based on dates if not explicitly set
        if 'metadata' in levels_data and 'levels' in levels_data:
            threshold_days = levels_data['metadata'].get('new_level_threshold_days', 30)
            current_date = datetime.now().date()
            
            for level in levels_data['levels']:
                if 'created_date' in level:
                    created_date = datetime.strptime(level['created_date'], '%Y-%m-%d').date()
                    days_old = (current_date - created_date).days
                    
                    # Auto-determine status if not set
                    if 'status' not in level:
                        level['status'] = 'new' if days_old <= threshold_days else 'old'
            
        return levels_data
    except (FileNotFoundError, json.JSONDecodeError):
        # Return compatible format with old levels.json
        return {'levels': levels, 'metadata': {'version': '1.0.0'}}

def get_levels_by_status(status_filter='all'):
    """Get levels filtered by status (new, old, or all)"""
    levels_data = load_levels_with_status()
    
    if status_filter == 'all':
        return levels_data.get('levels', [])
    
    return [level for level in levels_data.get('levels', []) 
            if level.get('status', 'old') == status_filter]

def get_level_status_summary():
    """Get summary of level status counts"""
    levels_data = load_levels_with_status()
    levels_list = levels_data.get('levels', [])
    
    new_count = sum(1 for level in levels_list if level.get('status') == 'new')
    old_count = sum(1 for level in levels_list if level.get('status') == 'old')
    
    return {
        'total': len(levels_list),
        'new': new_count,
        'old': old_count,
        'metadata': levels_data.get('metadata', {})
    }
...
def teacher_question_pools():
    ...
    pools_data = load_question_pools()
    level_status_summary = get_level_status_summary()
    
    # Get statistics for each pool
    for pool_name, pool in pools_data["pools"].items():
        pool_questions = get_questions_for_pool(pool_name)
        assigned_ids = pool.get("question_ids", [])
        
        # Calculate unassigned questions (available for assignment)
        total_in_system = len(questions)
        unassigned_questions = total_in_system - len(assigned_ids)
            
        pool["stats"] = {
            "total_questions": unassigned_questions,  # Questions available to assign
            "assigned_questions": len(assigned_ids),  # Questions already assigned
            "enabled": pool.get("enabled", True)
        }
        
        # Add level status information for level-based pool
        if pool_name == "level_based":
            pool["stats"]["new_levels"] = level_status_summary["new"]
            pool["stats"]["old_levels"] = level_status_summary["old"] 
            pool["stats"]["total_levels"] = level_status_summary["total"]
    
    return render_template('teacher_question_pools.html', 
                         pools=pools_data["pools"], 
                         metadata=pools_data["metadata"],
                         all_questions=questions,
                         level_status=level_status_summary)
...
def teacher_update_pool_settings():
    ...
    try:
        ...
        if pool_name == "test_yourself":
            ...
            pool["settings"]["show_level_status"] = request.form.get('show_level_status') == 'true'
            pool["settings"]["new_level_indicator"] = request.form.get('new_level_indicator') == 'true'
...
@app.route('/teacher/levels-by-status/<status>')
@teacher_required  
def teacher_levels_by_status(status):
    """Get levels filtered by status (new/old/all)"""
    try:
        if status not in ['new', 'old', 'all']:
            return jsonify({"error": "Invalid status filter"}), 400
            
        levels = get_levels_by_status(status)
        summary = get_level_status_summary()
