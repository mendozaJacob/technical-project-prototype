...
def safe_json_load(file_path, default_value=None):
    """Safely load JSON file with error handling and recovery"""
    try:
...
def safe_json_save(file_path, data):
    """Safely save JSON file with error handling and backup"""
    import os
    import shutil
...
def handle_application_error(error, context="Unknown"):
    ...
    if hasattr(error, '__traceback__'):
        print(f"[TRACE] {''.join(traceback.format_tb(error.__traceback__))}")
    
    # Flash user-friendly error message
    try:
        from flask import flash
        flash(f'An error occurred: {str(error)}. Please try again or contact support.', 'error')
    except:
        pass  # Flash may not be available in all contexts
...
def teacher_delete_chapter(chapter_id):
    try:
        ...
        chapters_data['chapters'] = [ch for ch in chapters if ch.get('id') != chapter_id]
        
        if len(chapters_data['chapters']) == original_count:
            return jsonify({'success': False, 'error': 'Chapter not found'}), 404
        
        # Update metadata
        from datetime import datetime
        chapters_data['metadata']['last_updated'] = datetime.now().isoformat()
        chapters_data['metadata']['total_chapters'] = len(chapters_data['chapters'])
...
def teacher_delete_question(question_id):
    try:
        ...
        all_questions = safe_json_load('data/questions.json', [])
        
        if not all_questions:
            return jsonify({'success': False, 'error': 'No questions found or unable to load questions file'})
        
        # Check if question exists
        question_exists = any(q.get('id') == question_id for q in all_questions)
        ...
        if len(all_questions) == original_count:
            return jsonify({'success': False, 'error': f'Failed to delete question {question_id}'})
        
        # Save questions safely
        if safe_json_save('data/questions.json', all_questions):
            # Update global questions variable
            global questions
            questions = all_questions
            return jsonify({'success': True, 'message': f'Question {question_id} deleted successfully'})
        else:
            return jsonify({'success': False, 'error': 'Failed to save updated questions file'})
        
    except Exception as e:
        error_msg = handle_application_error(e, f"delete question {question_id}")
...
def teacher_add_question():
    try:
        ...
        # Handle different question types
        if question_type == 'true_false':
            # For True/False, get answer from radio button
            answer = request.form.get('tf_answer', 'true')
            options = []
        elif question_type == 'multiple_choice':
            # For Multiple Choice, get options and correct answer
            options = [
            ...
            # Get correct answer (should match one of the options)
            answer = request.form.get('mc_answer', '').strip()
            if answer not in options and options:
                # If answer doesn't match options, use the first option
                answer = options[0]
        else:
            # For Short Answer
        ...
        if safe_json_save('data/questions.json', all_questions):
            ...
            flash(f'Question {next_id} added successfully!', 'success')
        else:
            flash('Error: Failed to save the new question. Please try again.', 'error')
        
        return redirect(url_for('teacher_questions'))
        
    except Exception as e:
        handle_application_error(e, "add question")
        return redirect(url_for('teacher_questions'))
...
def teacher_edit_question():
    try:
        ...
        if safe_json_save('data/questions.json', all_questions):
            ...
            flash('Error: Failed to save updated question. Please try again.', 'error')
        
        return redirect(url_for('teacher_questions'))
        
    except Exception as e:
        handle_application_error(e, f"update question {question_id}")
        return redirect(url_for('teacher_questions'))
...
def teacher_question_pools():
    ...
    for pool_name, pool in pools_data["pools"].items():
        pool_questions = get_questions_for_pool(pool_name)
        assigned_count = len(pool.get("question_ids", []))
        total_available = len(questions)  # Total questions in system
        unassigned_count = total_available - assigned_count
        
        pool["stats"] = {
            "total_questions": len(pool_questions),
            "assigned_questions": assigned_count,
            "unassigned_questions": unassigned_count,
            "total_in_system": total_available,
            "enabled": pool.get("enabled", True)
        }
